...
function checkModel(model, existModels) {
  ...
  log(namespace, '[app.model] namespace should be defined'); // 并且是字符串

  log(typeof namespace === 'string', "[app.model] namespace should be string, but got ".concat(_typeof(namespace))); // 并且唯一

  log(!existModels.some(function (model) {
    return model.namespace === namespace;
  }), '[app.model] namespace should be unique'); // state 可以为任意值
  // reducers 可以为空，PlainObject 或者数组

  if (reducers) {
    log(isPlainObject(reducers) || isArray(reducers), "[app.model] reducers should be plain object or array, but got ".concat(_typeof(reducers))); // 数组的 reducers 必须是 [Object, Function] 的格式

    log(!isArray(reducers) || isPlainObject(reducers[0]) && isFunction(reducers[1]), '[app.model] reducers with array should be [Object, Function]');
  } // effects 可以为空，PlainObject


  if (effects) {
    log(isPlainObject(effects), "[app.model] effects should be plain object, but got ".concat(_typeof(effects)));
  }

  if (subscriptions) {
    // subscriptions 可以为空，PlainObject
    log(isPlainObject(subscriptions), "[app.model] subscriptions should be plain object, but got ".concat(_typeof(subscriptions))); // subscription 必须为函数
...
function prefix(obj, namespace, type) {
  return Object.keys(obj).reduce(function (memo, key) {
    log(key.indexOf("".concat(namespace).concat(NAMESPACE_SEP)) !== 0, "[prefixNamespace]: ".concat(type, " ").concat(key, " should not be prefixed with namespace ").concat(namespace), 'warn');
...
function () {
  ...
  _createClass(Plugin, [{
    ...
    value: function use(plugin) {
      log(isPlainObject(plugin), 'plugin.use: plugin should be plain object');
      var hooks = this.hooks;

      for (var key in plugin) {
        if (Object.prototype.hasOwnProperty.call(plugin, key)) {
          log(hooks[key], "plugin.use: unknown plugin property: ".concat(key));
    ...
    value: function apply(key, defaultHandler) {
      ...
      log(validApplyHooks.indexOf(key) > -1, "plugin.apply: hook ".concat(key, " cannot be applied"));
      var fns = hooks[key];
      return function () {
        if (fns.length) {
    ...
    value: function get(key) {
      var hooks = this.hooks;
      log(key in hooks, "plugin.get: hook ".concat(key, " cannot be got"));
...
function getWatcher(key, _effect, model, onError, onEffect) {
  ...
  if (Array.isArray(_effect)) {
    ...
    if (opts && opts.type) {
      ...
      if (type === 'throttle') {
        log(opts.ms, 'app.start: opts.ms should be defined if type is throttle');
        ms = opts.ms;
      }
    }

    log(['watcher', 'takeEvery', 'takeLatest', 'throttle'].indexOf(type) > -1, 'app.start: effect type should be takeEvery, takeLatest, throttle or watcher');
  }
...
function createEffects(model) {
  function assertAction(type, name) {
    ...
    log(type.indexOf("".concat(model.namespace).concat(NAMESPACE_SEP)) !== 0, "[".concat(name, "] ").concat(type, " should not be prefixed with namespace ").concat(model.namespace), 'warn');
  }
...
function prefixedDispatch(dispatch, model) {
  return function (action) {
    var type = action.type;
    log(type, 'dispatch: action should be a plain Object with type');
    log(type.indexOf("".concat(model.namespace).concat(NAMESPACE_SEP)) !== 0, "dispatch: ".concat(type, " should not be prefixed with namespace ").concat(model.namespace), 'warn');
...
function unlisten(unlisteners, namespace) {
  ...
  log(nonFuncs.length === 0, "[app.unmodel] subscription should return unlistener function, check these subscriptions ".concat(nonFuncs.join(', ')), 'warn');
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;
...
function create() {
  ...
  function start() {
    ...
    log(Object.keys(extraReducers).every(function (key) {
...
function zus() {
  ...
  function router(_router) {
    log(isFn(_router), "[app.router] router should be function, but got ".concat(_typeof(_router)));
    app._router = _router;
  }
  ...
  function start(container) {
    ...
    if (isString(container)) {
      ...
      log(container, "[app.start] container ".concat(container, " not found"));
    } // 并且是 HTMLElement


    log(!container || isNode(container), '[app.start] container should be HTMLElement'); // 路由必须提前注册
