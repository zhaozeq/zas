...
function run(subs, model, app, onError) {
  ...
  for (var key in subs) {
    if (Object.prototype.hasOwnProperty.call(subs, key)) {
      var sub = subs[key];
      var unlistener = sub({
        dispatch: (0, _prefixedDispatch["default"])(app._store.dispatch, model) // history: app._history
...
function unlisten(unlisteners, namespace) {
  ...
  (0, _logTips["default"])(nonFuncs.length === 0, "[app.unmodel] subscription should return unlistener function, check these subscriptions ".concat(nonFuncs.join(', ')), 'warn');
